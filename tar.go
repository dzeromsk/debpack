...
// FromTar reads a tar file and creates an deb stuct.
func FromTar(inp io.Reader, md DEBMetaData) (*DEB, error) {
	r, err := NewDEB(md)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create DEB structure")
	}
	t := tar.NewReader(inp)
	for {
		h, err := t.Next()
